...
def audio():
    ...
    if function_name is not None:
        ...
        if function_name == "get_weather":
            ...
            final_response = "Esta funcionalidad aun no la has implementado gerly muahaha"
            tts_file = TTS().process(final_response)
            return {"result": "ok", "text": final_response, "file": tts_file}
        ...
        final_response = "No tengo idea de lo que estás hablando, Gerly, podrias ser mas especifica"
        tts_file = TTS().process(final_response)
        return {"result": "ok", "text": final_response, "file": tts_file}